function cutPoints = cutPointsForSubset(attribute,labels, first, last,N_0)
...
if last - first< 2
    ...
    if gain <= 0
        ...
        accept = FayyadAndIranisMDL(priorCounts,bestCounts,num_Inst,numCutPoints,N_0);
        if accept
            l = l+1;
            left = cutPointsForSubset(attribute,labels,first,bestIndex+1,N_0);
            right = cutPointsForSubset(attribute,labels,bestIndex+1,last,N_0);
